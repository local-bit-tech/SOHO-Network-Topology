...
We must turn off portfast or turn on bpduguard because if portfast is left on in an interface, there might be a broadcast storm that might form if there is partial mesh or full mesh topology that is formed with no STP guardrails.
...
## Trunk link between SW1 and R2

In order for ROAS (which we will see soon) to work properly, we have to ensure SW1 can send vlan traffic to R1. 
